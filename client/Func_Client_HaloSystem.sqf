Halo_Marker_LZ =
{
	if ((isServer) && (not(local player))) exitwith {};
...
Halo_Parachute =
{
	...
	if (_unit == player) then 
	{
		...
		waituntil {(position vehicle player select 2) < 1 || !isnil {player getvariable "bis_fnc_halo_terminate"}};
		...
		{
			switch (round(random 1)) do {case 0: {player playMoveNow "ActsPercMrunSlowWrflDf_TumbleOver";};case 1: {player playMoveNow "ActsPercMrunSlowWrflDf_FlipFlopPara";};};
		};
...
Halo_Jump =
{
	...
	if ((secondaryWeapon player) != "") exitwith {hint format ["You have too much gear\nRemove %1",secondaryWeapon player];	closedialog 0;};
