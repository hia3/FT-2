...
_period=_v getVariable "flareslasttime";
if (time<_period) exitWith {};
_v setVariable ["flareslasttime",time+3];
...
{	
	...
	{
		_relpos=if (_manual) then {_v modelToWorld [0, -3, -2]} else {_v modelToWorld (_v selectionposition format["flare_launcher%1",_i])};
		_dirpos=if (_manual) then {_v modelToWorld [0, -4, -2.5]} else {_v modelToWorld (_v selectionposition format["flare_launcher%1_dir",_i])};
